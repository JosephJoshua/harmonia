...
export class Chat extends AIChatAgent<Env, ChatState> {
        ...
        execute: async (dataStream) => {
          ...
          const processedMessages = (
            await processToolCalls({
              ...
              dataStream,
              tools: {...tools.health, ...tools.knowledge, ...tools.finance},
              executions,
            })
          ...
          try {
            const agentsInvolved = await this.agentRouter(
              openrouter,
              processedMessages
            );
            ...
            const result = streamText({
              ...
              messages: [
                {
                  role: "user",
                  content: `Original user input: ${this.messages[this.messages.length - 1]}`,
                },
  ...
  private async agentRouter(
    openrouter: OpenRouterProvider,
    query: Message[]
  ): Promise<("finance" | "health" | "knowledge")[]> {
    return (
      await generateObject({
        model: openrouter.chat(GEMINI_MODEL),
        output: "array",
        schema: z.enum(["finance", "health", "knowledge"]),
        system: `
